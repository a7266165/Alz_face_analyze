...
env 完整 requirement 檔在 `envs/*.txt`（12 份 pip freeze export）。`envs/README.md` 只文件化前三個 tool env，其他靠本備忘錄對照。
...
- 12 個 env 看起來多，但每個都鎖定某版 torch 或關鍵套件，硬統一必衝突（例：OpenFace 3 綁 timm 1.0.x，Py-Feat 綁 0.9.x）
