...
標籤：`[P]` = 有 PDF，`[T]` = 有 .txt 抽取，`[-]` = 只有 abstract（或無）。

5 條硬性需求（anchor: Chien 2023、Asgarian 2019、Taati 2019）：
- **R1** — Cohort 直接含 AD/MCI/dementia + 來源 + 具體 n
- **R2** — 純 face-image 模態（無 multimodal fusion）
- **R3** — 具體 AD vs HC 量化結果 + 統計檢定
- **R4** — face feature 來自 passive imaging / 自然觀察 / 既存 dataset frames（commanded movement task fail）
- **R5** — research question 是 AD biomarker 或 AD-cohort 上的 face-image methodology audit

排列：本檔按 review status — #1-2 核心、#3-4 方法論參考、#5-7 延伸-induction、#8-9 延伸-review、#10-12 邊緣。

共 12 篇（Ko 2021 / Pucciarelli 2018 dupe 已合併；Pucciarelli 2018 craniofacial 重建 thesis 已 archive）。

> **Backfill 紀錄**：Albaker 2023、Yoonesi 2025 的 abstract 為空，從 PDF 抽出文字補回 JSON。Chien 2023 透過 Europe PMC（PMC10604711）取得 OA PDF + .txt，2026-05-05 補回完整摘要。Cenacchi 2025 透過 Phase C lit-monitor 新關鍵字搜出（arXiv:2511.13802），2026-05-05 加入核心。

---


## ━━━━━━━━━━━━━━ 核心（2 篇 — 5/5 通過 5 條硬性需求 — 純臉部 + AD cohort + 具體結果 + 無刺激 + AD biomarker/audit）━━━━━━━━━━━━━━

## 1. [PT] **Chien 2023** — Analyzing Facial Asymmetry in Alzheimer's Dementia Using Image-Based Technology
...
## 2. [PT] **Cenacchi 2025** — Passive Dementia Screening via Facial Temporal Micro-Dynamics Analysis of In-the-Wild Talking-Head Video

> *arXiv:2511.13802*（Macquarie University, Sydney）

**背景**：現有失智篩檢仰賴 speech / scripted interview，限制在診間外，且 prediction 跟語言/轉錄綁死。本研究 target **passive 失智篩檢**：純 facial temporal micro-dynamics，無語言需求，跨 device / topic / culture transferable。

**方法**：

- **YT-DemTalk dataset**：自製從 YouTube 公開 in-the-wild 影片 curate，**300 clips（150 self-reported dementia + 150 controls）**。Talking-head 攝影、無 task / interview 干預、純自然臉部行為。
- **Features**：4 種 facial micro-kinematics — **blink dynamics**、small mouth/jaw motions、gaze variability、subtle head adjustments。Stabilize → time series → window-level activity-mix（motion 在各 stream 的相對比例，per-channel effects transparent）。
- **Classifier**：light-weight shallow classifiers + temperature scaling calibration。

**結果**：**AUROC 0.953、AP 0.961、F1 0.851、accuracy 0.857**。Ablation 確認 **gaze lability + mouth/jaw dynamics** 是最有資訊的 cue。

**結論**：純臉部微動力學 — 不靠語言/轉錄 — 能在 in-the-wild 影片做失智篩檢。提供 transcript-based pipeline 之外的可解釋部署選項。

> **5/5 通過**：純 face video（4 channels of facial kinematics）+ 自製 YT-DemTalk 300 clips + AUROC/AP/F1/acc 全給 + passive in-the-wild 觀察 + face-image 失智 biomarker。**Anchor paper for in-the-wild dementia screening + facial kinematics**（與 Chien 3D landmark 形成 anchor 對應 — Chien 用 static neutral 3D 圖、Cenacchi 用 dynamic in-the-wild video）。

## ━━━━━━━━━━━━━━ 方法論參考（2 篇 — R1 fail，無 AD cohort 但 R2-R5 過 — 方法論可借鏡）━━━━━━━━━━━━━━
...
## 4. [PT] **Yoonesi 2025** — Facial Expression Deep Learning Algorithms in the Detection of Neurological Disorders: A Systematic Review and Meta-Analysis
...
## ━━━━━━━━━━━━━━ 延伸-induction（3 篇 — R4 fail — paper 自行給 stimulus / task / commanded movement）━━━━━━━━━━━━━━
...
## ━━━━━━━━━━━━━━ 延伸-review（2 篇 — review papers — 延伸閱讀方法論全景）━━━━━━━━━━━━━━

## 8. [PT] **Al-baker 2023** — Accuracy and Reliability of Automated 3D Facial Landmarking in Medical and Biological Studies — A Systematic Review
...
## ━━━━━━━━━━━━━━ 邊緣（3 篇 — R3 fail（vague）或多項 fail — 保留但最低優先）━━━━━━━━━━━━━━

## 10. [--] **Al-baker 2025** — The Accuracy of Automated Facial Landmarking — A Comparative Study Between Cliniface Software and Patch-based CNN
...
## 11. [--] **Liu 2025** — Genetic Insights of Image-Based Traits: Analysis Pipeline for AI-based Phenotyping, Combined-GWAS, and Federated Learning with Application to the Human Face
...
## 12. [PT] **Yang 2025** — Motor Symptoms of Parkinson's Disease: Critical Markers for Early AI-Assisted Diagnosis
