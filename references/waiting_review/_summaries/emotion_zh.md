...
2026-04-29 由 abstract + 全文 PDF 整理。2026-05-05 套用 5 條硬性需求重新分桶。
...
排列：本檔按 review status — #1-3 核心、#4-12 方法論參考、#13-36 延伸-induction、#37-47 延伸-multimodal、#48-55 延伸-review、#56-66 邊緣。

共 **66 篇**（44 篇已歸檔到 _archive_rejected/）。
...
## ━━━━━━━━━━━━━━ 延伸-induction（24 篇 — R4 fail — paper 自行給 stimulus / task / intervention）━━━━━━━━━━━━━━
...
## 37. ★ [PT] **Lee 2024** — MCI Prediction with Multi-stream CNN

MCI 早期診斷成本/時間問題。**Multi-stream CNN** 從多模態資料偵測 MCI/失智。

## 38. ★ [PT] **Chen 2026** — MECO: Multimodal Dataset for Emotion + Cognitive Understanding in Older Adults

**42 位老人，~38 小時多模態訊號，30,592 同步 segment**。針對既有資料集少見的「老化族群 + 認知衰退影響表情」設計。**新公開 dataset**。

## 39. ★ [--] **R 2026** — Audio-Visual Disentangled Representation for Elderly CI Screening

社區規模 AD 篩檢需求。Audio-visual 共同訓 disentangled representation，可擴展非侵入 CI 評估。

## 40. ★ [PT] **Chen 2025** — Desktop App: Face Recognition + EEG for AD

便捷 AD 早期偵測工具：**MediaPipe 臉部分析 + Muse2 EEG + OpenAI API prompt** 整合的桌面程式。Pilot 級。

## 41. ◇ [PT] **Velichko 2025** — Multimodal Depression Detection (Semi-auto + Deterministic ML)

半自動標註 + 確定性 ML 多模態憂鬱偵測。語音 + 臉部表情。

## 42. ◇ [PT] **Camuñas 2025** — Affective Computing in Spanish-Speaking Older Adults Video Interviews

老人 + 非英語族群在情感運算缺資料。Spanish-speaking 老人視訊訪談評估 SOTA FER + 文字情感 + 微笑偵測。

## 43. ◇ [--] **Galanakis 2024** — MediaPipe Holistic for Predicting Aggressive Behavior in Dementia

**MediaPipe Holistic**（手勢 + 姿態 + 臉部）抓 landmark，分類失智者爭吵 vs 非爭吵行為。

## 44. ★ [--] **Shigekiyo 2024** — Anxiety Estimation in Dementia (Phrases + FE + Behaviors)

失智者焦慮 → BPSD（agitation 等）。早期 anxiety 偵測減輕 caregiver 負擔。**語句 + 臉部表情 + 行為**整合估算焦慮強度。

## 45. ★ [PT] **Agitha 2026** — NeuroSense: Multimodal Early Dementia Screening

整合語音 + EEG + 手寫 + 臉部情緒 + 認知測驗的 AI 多模態系統。每模態抽特徵後 ML 分類。

## 46. ★ [PT] **Maji 2024** — Gamified AI for Early Dementia Detection

整合**健康指標 CNN（1D）+ 臉部影像 CNN** 的遊戲化認知評估。1000 健康指標 + 1800 臉部影像資料訓練。

## 47. ★ [--] **Kadali 2024** — CNN AD Detection from FE + Eye Movements
...
## 48. ★ [PT] **Karako 2024** — Predictive DL for Cognitive Risk

從**易取得資料**做 MCI 預測 DL。減少需要病人主動做認知測驗的限制（症狀出現時可能太晚）。

## 49. ★ [PT] **Gaya-Morey 2025** — DL FER for Elderly Systematic Review
...
## 51. ○ [--] **Girard 2025** — Computational Analysis of Expressive Behavior in Clinical Assessment

概念性框架文：CV/語音/NLP 怎麼幫臨床心理評估增強信效度與規模化。

## 52. ★ [PT] **Alsuhaibani 2024** — Review DL for Non-Invasive CI Detection

Review：語音 + 語言 + 臉部 + 動作的 DL CI 偵測。**語音/語言基本上表現最高**，多模態結合（acoustic + linguistic）效益顯著。

## 53. ★ [PT] **Jiang 2024** — Contactless Detection Emotion + CI in Elderly Review

加拿大老化族群 ~11% 認知衰退。傳統診斷（MRI/PET/認知測驗）成本高。**Review 非接觸式偵測**（情緒 + CI）方法。

## 54. ◇ [PT] **Rymaszewski 2025** — ML for Biological Age Estimation Review

**生物年齡 (BA)** 比 chronological age 更能預測壽命/健康。Narrative review：ML 從低侵入資料源（**含臉部影像**、胸 X 光、腦 MRI、血液 biomarker、ECG、心理問卷）估算 BA 的方法。

## 55. ○ [PT] **Khare 2023** — Emotion Recognition + AI 2014-2023 Systematic Review

涵蓋物理（臉/語音）+ 生理（EEG/ECG/EDA/PPG）訊號的 AI 情緒辨識方法 systematic review。


## ━━━━━━━━━━━━━━ 邊緣（11 篇 — R3 fail（vague results）或多項 fail — 保留但最低優先）━━━━━━━━━━━━━━

## 56. ★ [--] **Zheng 2025** — Augmenting Face Mesh for Dementia Detection

語言 marker 受重視，臉部 marker 相對被忽略。**擴增臉部 mesh** 增強失智偵測 — 直接相關方法論進展。

## 57. ★ [--] **Alzahrani 2025** — Facial Cues for CI Detection from In-the-Wild Data

**眨眼率 (EBR)** + 頭部轉動率 (HTR) + 頭部動作統計特徵 (HMSF)。區分 ND/MCI/FMD/HC 的視覺特徵 in-the-wild 分析。

## 58. ○ [PT] **Fabrício 2024** — Brazilian Face DB for Basic Emotion FER

巴西族群：考慮年齡/性別/種族分布的臉部基本情緒資料庫建置 + 驗證。3 階段流程。

## 59. ○ [PT] **Veigas 2024** — DL FER for Elderly

老化族群心理健康挑戰。FER 偵測情緒供心理健康 monitoring。

## 60. ◇ [PT] **Ai 2025** — Service Robots Multimodal Emotion (Transfer Learning)

老人特殊情緒表達特性。Transfer learning 跨 AffectNet 等公開資料到老人領域，多模態情緒辨識給社交機器人。

## 61. ★ [PT] **Ruoranen 2023** — Memory Disorder Detection via FER

記憶障礙下的表情變化（特別是 apathy 降低表情強度）。FER 程式應用於記憶障礙篩檢的論文（thesis）。

## 62. ◇ [--] **M 2024** — Optimized Attention FER for ND Healthcare

CBAM 注意力模組 + 輕量 DLN 在 AD/PD 的 FER 應用。

## 63. ◇ [PT] **Gabrielli 2024** — Universal Time-Series Model for Stress Monitoring (ND)

ND 病人壓力監測：ECG + actigraphy + 語音 + **臉部分析**。穿戴裝置 HRV 不準的問題；通用時序模型整合多源訊號。

## 64. ○ [--] **Patrick 2024** — Initial Expressed Emotion During Neuropsych Assessment

神經心理測驗中**初始表達情緒**與認知測試表現的關聯（趨近/迴避 motivational dimension）。

## 65. ◇ [--] **Kolosov 2024** — AU + Emotions in Dementia Listening to Music

音樂介入失智照護。AU 與快樂程度的關聯（專家觀察）。**特定 AU 與快樂程度對應**。個人化音樂播放清單應用。

## 66. ★ [--] **Kanjalkar 2025** — AI Dementia Early Care + Prognosis
