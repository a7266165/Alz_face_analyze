...
## 9. ○ [PT] **Asgarian 2019** — Limitations of Facial Landmark Detection in Dementia Older Adults
...
## 11. ★ [PT] **Gaya-Morey 2025** — DL FER for Elderly Systematic Review

**31 篇近 10 年研究**的 systematic review。老人 FER 應用於 assisted living、心理健康、個人化照護。

## 12. ★ [--] **Kadali 2024** — CNN AD Detection from FE + Eye Movements

CNN 結合臉部表情 + 眼動分析做 AD 偵測。Image preprocessing 多種方法。

## 13. ★ [PT] **Ruoranen 2023** — Memory Disorder Detection via FER
...
## 16. ★ [PT] **Matsuda 2025** — Smile Detection in Real-World Dementia Care (QOL Pilot)

QOL 評估通常自評；失智進展後病人無法自評，改 proxy 評估。**真實照護場景的微笑偵測**作為客觀 QOL 評估的 pilot。

## 17. ★ [--] **Sun 2023** — MC-ViViT: MCI Detection from Facial Videos (I-CONECT)

**Multi-branch Classifier-ViViT**：抽 spatiotemporal 特徵 + 增強表徵。**用 I-CONECT 資料**。最高準確 90.63%（部分訪談視訊）。

## 18. ○ [--] **Girard 2025** — Computational Analysis of Expressive Behavior in Clinical Assessment

概念性框架文：CV/語音/NLP 怎麼幫臨床心理評估增強信效度與規模化。

## 19. ★ [PT] **Montenegro 2018** — AD Diagnosis via VR + Emotion Analysis (Thesis)

PhD thesis：早期 AD 非侵入診斷。整合認知方法（VR 環境）+ 情緒分析。

## 20. ★ [PT] **Sooriyaarachchi 2026** — Facial Expressions as Nexus for Health Assessment

評估視角：臉部表情作為**行為 phenotype**，在認知衰退/疼痛等健康障礙下偏離 baseline。Review ML 進展。

## 21. ★ [--] **Y 2023** — ML for Depression/Anxiety/Apathy in MCI (Speech + Facial)

MCI 患者高比例有 depression/anxiety/apathy；這些症狀預測 MCI→失智進展。觀察性研究：**語音 + 臉部表情**作 ML 模型 input。

## 22. ★ [PT] **Alsuhaibani 2024** — Review DL for Non-Invasive CI Detection

Review：語音 + 語言 + 臉部 + 動作的 DL CI 偵測。**語音/語言基本上表現最高**，多模態結合（acoustic + linguistic）效益顯著。

## 23. ★ [PT] **Jiang 2024** — Contactless Detection Emotion + CI in Elderly Review

加拿大老化族群 ~11% 認知衰退。傳統診斷（MRI/PET/認知測驗）成本高。**Review 非接觸式偵測**（情緒 + CI）方法。

## 24. ★ [PT] **Karako 2024** — Predictive DL for Cognitive Risk

從**易取得資料**做 MCI 預測 DL。減少需要病人主動做認知測驗的限制（症狀出現時可能太晚）。

## 25. ★ [--] **C 2024** — Multi-Dimensional Emotion Features for AD/MCI Diagnosis

長照 + 社區族群（HC=26 / MCI=23 / AD ≥60 歲）。**多維情緒特徵 + ML** 自動分類框架。

## 26. ★ [PT] **Takeshige-Amano 2024** — AD Detection via Smiles + Chatbot Conversations

AD 認知狀態評估迫切。**99 HC + 93 AD/MCI** 與 chatbot 對話、抽微笑影像 + 視聽特徵。臉 + 聲 + 對話內容多 cue。

## 27. ★ [--] **Dong 2025** — Facial Micro-expressions for MCI (Clock Drawing Test)

**畫鐘測驗**過程中錄製臉部視訊，分析 micro-expression 與認知功能的關聯，作為 MCI digital biomarker。

## 28. ★ [--] **J 2021** — Facial Emotion Mimicry in Older Adults With/Without AD

25 位老人（HC + MCI + AD）用 **Kinect 3D** 錄製臉部，比較 6 種基本情緒模仿（mimicking）的適切性，跟自身典型表情比對。

## 29. ◇ [--] **Matić 2026** — PD vs Depression FER Objectivization (Slovenian)

PD 的 hypomimia 跟憂鬱症臨床表型重疊難鑑別。**80 位（30 PD + 25 depression + 25 HC）**做 4 種 FE 任務（自發/自主/模仿/閱讀）。Hi-level Emotion 框架。

## 30. ★ [--] **M 2025** — Emotion Recognition in MCI/AD Rehabilitation
