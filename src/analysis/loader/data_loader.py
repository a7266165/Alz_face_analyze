...
class DataLoader:
    ...
    def _load_features(
        self, model: str, feature_type: str
    ...
    def _create_dataset(
        ...
        filtered_demo = self._filter_demographics(
            demographics=filtered_demo, cdr_threshold=cdr_threshold
        )
    ...
    def _filter_by_predicted_age(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"預測年齡篩選: {before} → {len(df)} 筆 (>= {self.min_predicted_age} 歲)"
        )
        return df

    def _filter_demographics(
        self, demographics: pd.DataFrame, cdr_threshold: float
    ) -> pd.DataFrame:
        """篩選人口學資料"""
        ...
        if cdr_threshold is not None:
            ...
            for group in df["group"].unique():
                ...
                if group == "ACS":
                    ...
                    if "Global_CDR" in df.columns:
                        mask.loc[group_mask] = (
                            df.loc[group_mask, "Global_CDR"] <= cdr_threshold
                        ).values
                elif group == "P":
                    if "Global_CDR" in df.columns:
                        mask.loc[group_mask] = (
    ...
    def _apply_data_balancing(self, demographics: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            age_bins = pd.qcut(all_ages, q=self.n_bins, duplicates="drop", retbins=True)[1]
        except ValueError:
            n_bins_effective = min(self.n_bins, all_ages.nunique())
            age_bins = pd.qcut(
                all_ages, q=n_bins_effective, duplicates="drop", retbins=True
            )[1]
        ...
        for bin_val in health_group["age_bin"].cat.categories:
            ...
            if majority_is_health:
                ...
                if n_health_bin > target_health:
                    health_sample = health_in_bin.sample(
                        n=target_health, random_state=rng
                    )
                else:
                ...
                if n_patient_bin > target_patient:
                    patient_sample = patient_in_bin.sample(
                        n=target_patient, random_state=rng
                    )
                else:
    ...
    def _calculate_filter_stats(self) -> Dict:
        ...
        return {
            ...
            "filtered_out_ratio": (
                filtered_out_total / total_original if total_original > 0 else 0
            ),
            "health_original": int(health_original),
            "health_filtered_out": int(health_filtered_out),
            "health_filtered_out_ratio": (
                health_filtered_out / health_original if health_original > 0 else 0
            ),
            "patient_original": int(patient_original),
            "patient_filtered_out": int(patient_filtered_out),
            "patient_filtered_out_ratio": (
                patient_filtered_out / patient_original if patient_original > 0 else 0
            ),
        }
