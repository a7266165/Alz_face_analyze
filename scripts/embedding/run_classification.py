...
``workspace_refactor_20260601``(EMBEDDING_CLASSIFICATION_REFACTOR_DIR),**絕不與 legacy 混**。

正交軸(互不干涉):
  - cohort 4-token(``--p-visit/--p-score/--hc-visit/--hc-score``):決定族群,順序同 cohort_list。
  - ``--variant``:feature type(original / absolute_difference …),只決定載哪個 X。
...
用法:
    python scripts/embedding/run_classification.py \\
...
def main():
    ...
    ap.add_argument("--variant", default="original",
                    help="feature type:original / absolute_difference / …。"
                         "只決定載哪個特徵,與 --model 正交")
    ...
    run_cell(cohort, args.bg_mode, args.embedding, args.variant,
             ...
             lr_C=args.lr_C, output_root=args.output_root)
    logger.info("done.")
