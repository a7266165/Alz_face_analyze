"""Shared constants and drawing helpers for the age/embedding pipeline diagrams."""
import matplotlib.pyplot as plt
from matplotlib.patches import FancyBboxPatch
from pathlib import Path

__all__ = [
    'OUT', 'FONT', 'FS', 'EC', 'NODE_H', 'SP',
    'C1', 'C2', 'C3', 'C4', 'C6', 'CF', 'CR',
    'C_FT', 'C_PA', 'C_PD', 'C_SA', 'C_ES', 'C_ML', 'C_MS',
    'C_AGE', 'C_PRE', 'C_EACS', 'C_AOUT', 'C_ASY', 'C_EMO',
    'AGE_MODELS', 'NW_AM', 'GAP_AM', 'TOTAL_AM', 'X_AGE', 'NW_PRE',
...
# Palettes: bg = cluster wrapper fill, nd = node fill.
...
def node(ax, cx, cy, w, h, text, fc, fs=FS, fc_text='#212121'):
    """Filled box with a centred label."""
...
def line(ax, x1, y1, x2, y2, color=EC, lw=1.4, zorder=2):
    """Straight connector between two points (zorder<0 routes it under cluster fills)."""
    ax.plot([x1, x2], [y1, y2], color=color, lw=lw,
            solid_capstyle='round', zorder=zorder)


def rowx(cx, n, w, gap):
    """Return (x-positions, total-width) for n nodes of width w centred on cx."""
    tot = n * w + (n - 1) * gap
