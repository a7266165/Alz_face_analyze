"""v4: Four-branch pipeline diagram (Age + Embedding + Asymmetry + Emotion).

Output:
  workspace/overview/age_emb_pipeline_mpl_v4.png

Usage:
    python scripts/overview/draw_age_emb_pipeline_v4.py
"""
...
def build_v4():
    """v4: Age + Embedding + Asymmetry + Emotion (4-branch pipeline)."""
    # ── Position constants ──
    ...
    # ════════════════════════════════════
    # Shared Cohort (at CX_S) — 6 rows
    # ════════════════════════════════════
    cluster(ax, CX_S, (c1_top + c1_bot) / 2,
            c1_w, c1_bot - c1_top, C1['bg'])

    # Row 1: Demographic
    node(ax, CX_S, y_dem, nw_dem, NODE_H, 'Demographic', C1['nd'])

    # Row 2: P visit
    pvx = [CX_S - (nw_pv + gap_pv) / 2, CX_S + (nw_pv + gap_pv) / 2]
    for x, lab in zip(pvx, ['P: first', 'P: all']):
        node(ax, x, y_pv, nw_pv, NODE_H, lab, C1['nd'])
    for px in pvx:
        line(ax, CX_S, y_dem + NODE_H / 2, px, y_pv - NODE_H / 2)

    # Row 3: HC visit
    hvx = [CX_S - (nw_pv + gap_pv) / 2, CX_S + (nw_pv + gap_pv) / 2]
    for x, lab in zip(hvx, ['HC: first', 'HC: all']):
        node(ax, x, y_hv, nw_pv, NODE_H, lab, C1['nd'])
    for px in pvx:
        for hx in hvx:
            line(ax, px, y_pv + NODE_H / 2, hx, y_hv - NODE_H / 2)
    ...
    for x, lab in zip(psx, P_SCORE_LABELS):
        node(ax, x, y_ps, nw_ps, NODE_H, lab, C1['nd'])
    for hx in hvx:
        for sx in psx:
            line(ax, hx, y_hv + NODE_H / 2, sx, y_ps - NODE_H / 2)
    ...
    for x, lab in zip(hsx, HC_SCORE_LABELS):
        node(ax, x, y_hs, nw_hs, NODE_H, lab, C1['nd'])
    for sx in psx:
        for hx2 in hsx:
            line(ax, sx, y_ps + NODE_H / 2, hx2, y_hs - NODE_H / 2)

    # Row 6: Cohort
    node(ax, CX_S, y_fc, nw_co, NODE_H,
    ...
    # ── Save ──
    out = OUT / "age_emb_pipeline_mpl_v4.png"
...
def build_v4_show():
    ...
    _n(ax, CX_S, y_dem, nw_dem, NODE_H, 'Demographic', C1['nd'])

    pvx = [CX_S - (nw_pv + gap_pv) / 2, CX_S + (nw_pv + gap_pv) / 2]
    for x, lab, on in zip(pvx, ['P: first', 'P: all'], [True, False]):
        _n(ax, x, y_pv, nw_pv, NODE_H, lab, C1['nd'], on)
    for px in pvx:
        line(ax, CX_S, y_dem + NODE_H / 2, px, y_pv - NODE_H / 2)

    hvx = [CX_S - (nw_pv + gap_pv) / 2, CX_S + (nw_pv + gap_pv) / 2]
    for x, lab in zip(hvx, ['HC: first', 'HC: all']):
        _n(ax, x, y_hv, nw_pv, NODE_H, lab, C1['nd'])
    for px in pvx:
        for hx in hvx:
            line(ax, px, y_pv + NODE_H / 2, hx, y_hv - NODE_H / 2)
    ...
    psx = [CX_S + (i - 1.5) * (nw_ps + gap_ps) for i in range(4)]
    P_SCORE_LABELS = ['P: CDR all', 'P: CDR >=0.5', 'P: CDR >=1', 'P: CDR >=2']
    for x, lab, on in zip(psx, P_SCORE_LABELS,
                          [False, True, False, False]):
        _n(ax, x, y_ps, nw_ps, NODE_H, lab, C1['nd'], on)
    for hx in hvx:
        for sx in psx:
            line(ax, hx, y_hv + NODE_H / 2, sx, y_ps - NODE_H / 2)
    ...
    HC_SCORE_LABELS = ['HC: CDR all\nor MMSE all',
                       'HC: CDR =0\nor MMSE >=26']
    for x, lab, on in zip(hsx, HC_SCORE_LABELS, [True, False]):
        _n(ax, x, y_hs, nw_hs, NODE_H, lab, C1['nd'], on)
    for sx in psx:
        for hx2 in hsx:
            line(ax, sx, y_ps + NODE_H / 2, hx2, y_hs - NODE_H / 2)

    _n(ax, CX_S, y_fc, nw_co, NODE_H,
       "Cohort (P + NAD + ACS)", C1['nd'])
    for hx2 in hsx:
        line(ax, hx2, y_hs + NODE_H / 2, CX_S, y_fc - NODE_H / 2)
    ...
    # ── Save ──
    out = OUT / "age_emb_pipeline_mpl_v4_show.png"
    fig.savefig(out, dpi=150, bbox_inches='tight',
                pad_inches=0.15, facecolor='white')
    plt.close(fig)
    print(f"Saved: {out}")
...
def build_v4_refactor():
    """v4 refactor view: only the Cohort cluster lit, everything else grayed.

    Same geometry as build_v4(); the whole pipeline is drawn in gray and just
    the 6-row Cohort section is shown in full color.

    Output:
      workspace/overview/age_emb_pipeline_mpl_v4_refactor.png
    """
    CX_S = 20.0; CX_E = 14.5; X_A = 26.0; X_EMO = 42.0
    FIG_W_V3 = 56.0
    age_w = 4.5; age_h = NODE_H
    ...
    # ── lit-toggle wrappers: cohort drawn lit, everything else grayed ──
    lit = [True]
    ...
    # ════════════════════════════════════
    # Shared Cohort (at CX_S) — 6 rows  [LIT]
    # ════════════════════════════════════
    _cl(ax, CX_S, (c1_top + c1_bot) / 2,
        c1_w, c1_bot - c1_top, C1['bg'])
    ...
    pvx = [CX_S - (nw_pv + gap_pv) / 2, CX_S + (nw_pv + gap_pv) / 2]
    for x, lab in zip(pvx, ['P: first', 'P: all']):
        _n(ax, x, y_pv, nw_pv, NODE_H, lab, C1['nd'])
    for px in pvx:
        line(ax, CX_S, y_dem + NODE_H / 2, px, y_pv - NODE_H / 2)
    ...
    P_SCORE_LABELS = ['P: CDR all', 'P: CDR >=0.5', 'P: CDR >=1', 'P: CDR >=2']
    for x, lab in zip(psx, P_SCORE_LABELS):
        _n(ax, x, y_ps, nw_ps, NODE_H, lab, C1['nd'])
    for hx in hvx:
        for sx in psx:
            line(ax, hx, y_hv + NODE_H / 2, sx, y_ps - NODE_H / 2)
    ...
    HC_SCORE_LABELS = ['HC: CDR all\nor MMSE all',
                       'HC: CDR =0\nor MMSE >=26']
    for x, lab in zip(hsx, HC_SCORE_LABELS):
        _n(ax, x, y_hs, nw_hs, NODE_H, lab, C1['nd'])
    for sx in psx:
        for hx2 in hsx:
            line(ax, sx, y_ps + NODE_H / 2, hx2, y_hs - NODE_H / 2)
    ...
    eacs_w = age_w + 0.6
    eacs_h = NODE_H * 1.6
    _cl(ax, X_AGE, y_fc, eacs_w + 0.6, eacs_h + 2 * SP, C_EACS['bg'])
    _n(ax, X_AGE, y_fc, eacs_w, eacs_h,
       "External Datasets\nUTKFace / AgeDB / APPA-REAL / IMDB\n"
       "MegaAge / FairFace / SZU-EmoDage\nAFAD / DiverseAsian",
       C_EACS['nd'])
    ...
    _cl(ax, CX_S, (c_emb_top + c_emb_bot) / 2,
        total_emb + 0.9, c_emb_bot - c_emb_top, C2['bg'])
    for x, lab in zip(emx, ['dlib', 'TopoFR', 'ArcFace', 'VGGFace']):
        _n(ax, x, y_emb, nw_emb, NODE_H, lab, C2['nd'])
    for src in [x_face, x_mirr]:
        for ex in emx:
            line(ax, src, y_mir + NODE_H / 2, ex, y_emb - NODE_H / 2)
    ...
    _cl(ax, X_AGE, y_emb, pred_cw, am_bot - am_top, C_AGE['bg'])
    for x, lab in zip(amx, AGE_MODELS):
        _n(ax, x, y_emb, NW_AM, age_h, lab, C_AGE['nd'])
    for x in amx:
        line(ax, x_face, y_mir + NODE_H / 2, x, y_emb - NODE_H / 2)
    for x in amx:
        line(ax, X_AGE, y_fc + NODE_H / 2, x, y_emb - age_h / 2)
    ...
    _cl(ax, X_AGE, y_a_out, total_out + 0.7,
        c_ao_bot - c_ao_top, C_AOUT['bg'])
    for x, lab in zip(ox, out_labels):
        _n(ax, x, y_a_out, out_w, NODE_H, lab, C_AOUT['nd'])
    for x in ox:
        line(ax, X_AGE, y_cal3 + age_h / 2, x, y_a_out - NODE_H / 2)
    ...
    pax = [CX_E - (nw_pa + gap_pa) / 2, CX_E + (nw_pa + gap_pa) / 2]
    for x, lab in zip(pax, ['mean', 'all']):
        _n(ax, x, y_pa, nw_pa, NODE_H, lab, C_PA['nd'])
    for px in pax:
        line(ax, CX_E, y_ft + NODE_H / 2, px, y_pa - NODE_H / 2)
    ...
    pdx = [CX_E - (nw_pd + gap_pd), CX_E, CX_E + (nw_pd + gap_pd)]
    for x, lab in zip(pdx, ['no_drop', 'PCA', 'DropCorr']):
        _n(ax, x, y_pd, nw_pd, NODE_H, lab, C_PD['nd'])
    for px in pax:
        for dx in pdx:
            line(ax, px, y_pa + NODE_H / 2, dx, y_pd - NODE_H / 2)
    ...
    _n(ax, xl, y_r3, nw_col, NODE_H,
       "Full Cohort Eval\nMatched Subset Eval (1:1)", CF['nd'])
    _cl(ax, xr, (cl_top + cl_bot) / 2, cw, cl_h, CR['bg'])
    _n(ax, xr, y_r1, nw_col, NODE_H, "Matched Cohort", CR['nd'])
    _n(ax, xr, y_r2, nw_col, NODE_H, "Predict Full Cohort", CR['nd'])
    _n(ax, xr, y_r3, nw_col, NODE_H,
       "Matched OOF Eval\nUnmatched Eval", CR['nd'])
    for ci in clx:
        line(ax, ci, y_clf + NODE_H / 2, xl, y_r1 - NODE_H / 2)
    ...
    _n(ax, xl_a, y_r3, nw_col, NODE_H,
       "Full Cohort Eval\nMatched Subset Eval (1:1)", CF['nd'])
    _cl(ax, xr_a, (cl_top + cl_bot) / 2, cw, cl_h, CR['bg'])
    _n(ax, xr_a, y_r1, nw_col, NODE_H, "Matched Cohort", CR['nd'])
    _n(ax, xr_a, y_r2, nw_col, NODE_H, "Predict Full Cohort", CR['nd'])
    _n(ax, xr_a, y_r3, nw_col, NODE_H,
       "Matched OOF Eval\nUnmatched Eval", CR['nd'])
    for sx in scx_r:
        line(ax, sx, y_pd + NODE_H / 2, xl_a, y_r1 - NODE_H / 2)
    ...
    _cl(ax, X_EMO, y_emb, total_emo + 0.9,
        emo_m_bot - emo_m_top, C_EMO['bg'])
    for x, lab in zip(emo_mx, emo_models):
        _n(ax, x, y_emb, mnw_emo, NODE_H, lab, C_EMO['nd'])
    for x in emo_mx:
        line(ax, x_face, y_mir + NODE_H / 2, x, y_emb - NODE_H / 2)
    ...
    _cl(ax, va_center, (c_ft_top + c_ft_bot) / 2,
        total_va + 0.6, c_ft_bot - c_ft_top, C_EMO['bg'])
    for x, lab in zip(va_xs, va_labels):
        _n(ax, x, y_ft, emo_fnw, NODE_H, lab, C_EMO['nd'])
    _cl(ax, ct_center, (c_ft_top + c_ft_bot) / 2,
        total_ct + 0.6, c_ft_bot - c_ft_top, C_EMO['bg'])
    for x, lab in zip(ct_xs, contempt_labels):
        _n(ax, x, y_ft, emo_fnw, NODE_H, lab, C_EMO['nd'])
    ...
    for br_cx in [X_AGE, CX_E, X_A, X_EMO]:
        ...
        for x, lab in zip(ppx, ['AD vs HC', 'AD vs NAD', 'AD vs ACS',
                                 'MMSE hi/lo', 'CASI hi/lo']):
            _n(ax, x, y_p, nw6, NODE_H, lab, C6['nd'])
        for mx_i in msx:
            for px in ppx:
                line(ax, mx_i, y_ms + NODE_H / 2, px, y_p - NODE_H / 2)

    # ── Save ──
    out = OUT / "age_emb_pipeline_mpl_v4_refactor.png"
    fig.savefig(out, dpi=150, bbox_inches='tight',
                pad_inches=0.15, facecolor='white')
    plt.close(fig)
    print(f"Saved: {out}")
